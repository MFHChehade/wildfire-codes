...
P_nk = sdpvar(120,1); % decision variable
P_ng = sdpvar(73,1); % decision variable

z = binvar(120,1); % decision variable
...
A = zeros(73,120); % node power flow net injection matrix
for i=1:73
    for j=1:120
        if(branch(j,1) == i)
            A(i,j) = 1;
...
constraints{end+1} = ones(1,120)*(ones(120,1) - z) <= 3;
